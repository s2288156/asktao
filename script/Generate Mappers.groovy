...
def generate(table, dir) {
    ...
    new File(dir, className + "Mapper" + ".java").withPrintWriter("utf-8") { out -> generate(out, table, className, fields) }
}
...
def generate(out, table, className, fields) {
    ...
    out.println "import com.muyuan.zhcl.auth.dataobject." + doName + ";"
    out.println ""
    out.println "/**"
    out.println " * @author wcy-auto"
    out.println " **/"

    out.println "public interface $cName extends BaseMapper<$doName> {"
    out.println ""
    out.println "}"
}
